...
%           TINC      time increment in seconds [default 0.45/BW]
%           ANN       annotation cell array: each row contains either
%                     {time 'text-string' 'font'} or {[t_start t_end] 'text-string' 'font'} where
%                     the time value is in seconds with s(n) at time n/fs. The font column can
...
%
% Outputs:  T(NT)        time axis values (in seconds). Input sample s(n) is at time n/fs.
...
%           B(NF,NT)        spectrogram values in dB
%
...
%%%% BUGS %%%%%%
% ann input argument not implemented
% interpolation needed at low frequencies for bark etc.
% future options:
...
%      Version: $Id: spgrambw.m,v 1.9 2010/01/21 21:26:07 dmb Exp $
%
...
ninc=max(round(tinc*fs),1);                 % window increment in samples
...
if ~nargout || any(mode=='g')
    ...
    if  any(dotaw)
        ...
        if dotaw(3)         % Plot the waveform
            ...
            srange=smax-smin;
            hold on
            plot((1:length(s))/fs,(s-smin)/srange*0.9*(toptaw(4)-toptaw(3))+toptaw(3),'color',map(48,:))
            hold off
        end
        if dotaw(1)  % draw time markers
            tmk=cell2mat(ann(:,1));
            ...
            switch size(tmk,2)
                ...
                case 1      % isolated marks
                    hold on
                    plot([tmk tmk]',repmat(ymk',1,numel(tmk)),'color',map(48,:));
                    hold off
                otherwise % draw durations
                    hold on
                    plot(tmk(:,[1 1 2 2])',repmat(ymk([1 2 2 1])',1,size(tmk,1)),'color',map(48,:));
                    hold off
            end
        end
        if dotaw(2)
